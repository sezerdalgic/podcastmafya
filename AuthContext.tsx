...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    setLoading(true); // START RACE CONDITION FIX
    try {
      await signInWithEmailAndPassword(auth, email, password);
      // The onAuthStateChanged listener will handle setting user and setting loading to false.
      return true;
    } catch (error) {
  ...
  return (
    <AuthContext.Provider value={{
      user, usersList, login, logout, isAuthenticated: !!user, loading,
